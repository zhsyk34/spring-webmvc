...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
    ...
    protected WebApplicationContext initWebApplicationContext() {
        ...
        WebApplicationContext currentContext = null;

        if (this.webApplicationContext != null) {
            // A context instance was injected at construction time -> use it
            currentContext = this.webApplicationContext;
            if (currentContext instanceof ConfigurableWebApplicationContext) {
                ConfigurableWebApplicationContext configContext = (ConfigurableWebApplicationContext) currentContext;
                if (!configContext.isActive()) {
                    // The context has not yet been refreshed -> provide services such as
                    // setting the parent context, setting the application context id, etc
                    if (configContext.getParent() == null) {
                        // The context instance was injected without an explicit parent -> set
                        // the root application context (if any; may be null) as the parent
                        configContext.setParent(rootContext);
                    }
                    configureAndRefreshWebApplicationContext(configContext);
                }
            }
        }
        if (currentContext == null) {
            ...
            currentContext = findWebApplicationContext();
        }
        if (currentContext == null) {
            // No context instance is defined for this servlet -> create a local one
            currentContext = createWebApplicationContext(rootContext);
        }
        ...
        if (!this.refreshEventReceived) {
            ...
            onRefresh(currentContext);
        }

        if (this.publishContext) {
            // Publish the context as a servlet context attribute.
            String attrName = getServletContextAttributeName();
            getServletContext().setAttribute(attrName, currentContext);
